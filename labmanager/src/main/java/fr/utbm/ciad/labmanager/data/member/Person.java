...
import fr.utbm.ciad.labmanager.data.*;
...
@Entity
@Table(name = "Persons")
public class Person extends AbstractContextData implements JsonSerializable, AttributeProvider, Comparable<Person>, IdentifiableEntity {
    ...
    public final URL getGithubAvatarURL(int size) {
        if (!Strings.isNullOrEmpty(getGithubId())) {
            try {
                return new URI(GITHUB_AVATAR_URL + getGithubId() + "?" + GITHUB_AVATAR_SIZE_PARAM + "=" + size).toURL(); //$NON-NLS-1$ //$NON-NLS-2$
            } catch (Throwable ex) {
                //
            }
        }
    ...
    public final URL getGoogleScholarAvatarURL(int size) {
        if (!Strings.isNullOrEmpty(getGoogleScholarId())) {
            try {
                return new URI(GOOGLE_SCHOLAR_AVATAR_URL + getGoogleScholarId() + "&" + GOOGLE_SCHOLAR_AVATAR_SIZE_PARAM + "=" + size).toURL(); //$NON-NLS-1$ //$NON-NLS-2$
            } catch (Throwable ex) {
                //
            }
        }
    ...
    public final URL getGravatarURL(int size) {
        ...
        if (size > 0 && !Strings.isNullOrEmpty(id)) {
            try {
                return new URI(GRAVATAR_URL + id + "?" + GRAVATAR_SIZE_PARAM + "=" + size).toURL(); //$NON-NLS-1$ //$NON-NLS-2$
            } catch (Throwable ex) {
                //
            }
        }
    ...
    public void setPrivateBiography(Boolean privateFlag) {
        this.privateBiography = privateFlag != null && privateFlag.booleanValue();
    }
